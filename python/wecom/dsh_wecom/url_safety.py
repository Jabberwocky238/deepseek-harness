...
The check can be globally disabled via ``DSH_WECOM_ALLOW_PRIVATE_URLS=true``
for environments where DNS resolves external domains to private/benchmark-range
IPs (OpenWrt routers, corporate proxies, VPNs that use 198.18.0.0/15 or
100.64.0.0/10).  Even when disabled, cloud metadata hostnames
(metadata.google.internal, 169.254.169.254) are **always** blocked — those are
never legitimate agent targets.

Limitations:
  - DNS rebinding (TOCTOU): an attacker-controlled DNS server with TTL=0
    can return a public IP for the check, then a private IP for the actual
    connection. Direct httpx request paths should use
  ...
  - Redirect-based bypass is mitigated by the httpx event hook in
    :mod:`.net`, which re-validates each redirect target. Both halves must
    stay installed on any client fetching a platform-supplied URL.
"""
...
from .helpers import is_truthy_value
...
# ---------------------------------------------------------------------------
# Cached after first read so we don't re-read the environment on every check.
_allow_private_resolved = False
_cached_allow_private: bool = False


def _global_allow_private_urls() -> bool:
    """Return True when ``DSH_WECOM_ALLOW_PRIVATE_URLS`` opts out of private-IP blocking.

    Cloud metadata endpoints stay blocked regardless of this toggle. The result
    is cached for the process lifetime.

    :returns: True when private-address resolution is permitted.
    """
...
def _resolve_allow_private_urls() -> bool:
    ...
    return is_truthy_value(os.getenv("DSH_WECOM_ALLOW_PRIVATE_URLS", ""), default=False)
...
def is_safe_url(url: str) -> bool:
    ...
    ``DSH_WECOM_ALLOW_PRIVATE_URLS=true``), private-IP blocking is skipped.
...
def ssrf_safe_async_http_transport(**kwargs: Any) -> Any:
    ...
    schemes_by_origin_var = contextvars.ContextVar("dsh_ssrf_async_origin_schemes")
...
def ssrf_safe_http_transport(**kwargs: Any) -> Any:
    ...
    schemes_by_origin_var = contextvars.ContextVar("dsh_ssrf_origin_schemes")
...
def _install_ssrf_guard_on_async_transport(transport: Any, schemes_by_origin_var: Any) -> None:
    ...
    if transport is None or state.get("_dsh_ssrf_guarded", False):
        return
    ...
    transport._dsh_ssrf_guarded = True
